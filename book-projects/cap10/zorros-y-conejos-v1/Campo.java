...
public class Campo
{
     ...
     * @param fila Coordenada de la fila de la posición.
     * @param columna Coordenada de la columna de la posición.
     */
    public void ubicar(Object animal, int fila, int columna)
    {
        ubicar(animal, new Ubicacion(fila, columna));
    }
     ...
     */
    public void ubicar(Object animal, Ubicacion posicion)
    {
        campo[posicion.getFila()][posicion.getColumna()] = animal;
    }
     ...
     */
    public Object getObjectoEn(Ubicacion posicion)
    {
        return getObjectoEn(posicion.getFila(), posicion.getColumna());
    }
     ...
     * @return El animal ubicado en la posicion, o null si no hay uno.
     */
    public Object getObjectoEn(int fila, int columna)
    {
     ...
     */
    public Ubicacion direccionAdyacentePorAzar(Ubicacion posicion)
    {
     ...
     */
    public Ubicacion direccionAdyacenteLibre(Ubicacion posicion)
    {
        Iterator<Ubicacion> adyacente = direccionesAdyacentes(posicion);
        ...
        if(campo[posicion.getFila()][posicion.getColumna()] == null) {
            return posicion;
        } 
        else {
            return null;
     ...
     */
    public Iterator<Ubicacion> direccionesAdyacentes(Ubicacion posicion)
    {
        int fila = posicion.getFila();
        int columna = posicion.getColumna();
        List<Ubicacion> posiciones = new LinkedList<Ubicacion>();
            ...
            if(filaSiguiente >= 0 && filaSiguiente < largo) {
                for(int deltaColumna = -1; deltaColumna <= 1; deltaColumna++) {
                    ...
                    if(columnaSiguiente >= 0 && columnaSiguiente < ancho && (deltaFila != 0 || deltaColumna != 0)) {
                        posiciones.add(new Ubicacion(filaSiguiente, columnaSiguiente));
                    }
                }
            }
        }
